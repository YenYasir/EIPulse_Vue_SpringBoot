...
public class SysUserController extends BaseController {
	...
	public AjaxResult export(SysUser user) {
		...
		return util.exportExcel(page.getContent(), "員工數據");
	}
	...
	public AjaxResult getInfo(@PathVariable(value = "userId", required = false) Long userId) {
		...
		if (StringUtils.isNotNull(userId)) {
			ajax.put(AjaxResult.MsgCode.DATA_TAG.getCode(), userService.selectUserById(userId));
	...
	public AjaxResult add(@Validated @RequestBody SysUser user) {
		if (UserConstants.NOT_UNIQUE.equals(userService.checkUserNameUnique(user.getUserName()))) {
			return AjaxResult.error("新增員工'" + user.getUserName() + "'失敗，登入帳號已存在");
		} else if (UserConstants.NOT_UNIQUE.equals(userService.checkPhoneUnique(user))) {
			return AjaxResult.error("新增員工'" + user.getUserName() + "'失敗，手機號碼已存在");
		} else if (UserConstants.NOT_UNIQUE.equals(userService.checkEmailUnique(user))) {
			return AjaxResult.error("新增員工'" + user.getUserName() + "'失敗，信箱帳號已存在");
		}
	...
	public AjaxResult edit(@Validated @RequestBody SysUser user) {
		...
		if (UserConstants.NOT_UNIQUE.equals(userService.checkPhoneUnique(user))) {
			...
			return AjaxResult.error("修改員工'" + user.getUserName() + "'失敗，信箱帳號已存在");
		}
