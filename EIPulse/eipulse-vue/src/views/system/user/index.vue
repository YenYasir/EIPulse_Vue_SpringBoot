...
export default {
    ...
    return {
      ...
      rules: {
        userName: [
          ...
          { min: 2, max: 20, message: '員工名稱長度必須介於 2 和 20 之間', trigger: 'blur' }
        ],
        nickName: [
        ...
        password: [
          ...
          { min: 5, max: 20, message: '員工密碼長度必須介於 5 和 20 之間', trigger: 'blur' }
        ],
        email: [
          {
            type: "email",
  ...
  methods: {
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm('確認要"' + text + '""' + row.userName + '"員工嗎？').then(function() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否確認刪除員工編號為"' + userIds + '"的數據項？').then(function() {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否確認導出所有員工數據項？').then(() => {
