...
public class AttendanceService {
	...
	@Autowired
	public AttendanceService(AttendanceRepository attendanceRepository, ClockTimeRepository clockTimeRepository,
	...
	// 設定出勤狀態碼
	private void updateAttendanceStatus(Attendance attendance, LocalDateTime firstClockTime,
			LocalDateTime lastClockTime, BigDecimal totalHours) {
	...
	// 依區間抓取單員工出勤狀況，最大為單月
	public Page<AttendanceDTO> findByDateBetweenEmpAttendance(LocalDate startDate, LocalDate endDate, Integer empId,
			Integer pageNumber) {
		Pageable pageable = PageRequest.of(pageNumber - 1, 8, Sort.Direction.ASC, "date");
		Page<Attendance> attendancePage = attendanceRepository.findByDateBetweenEmpAttendance(startDate, endDate, empId,
				pageable);
	...
	public Page<AttendanceDTO> findByMonthAllEmpAttendance(LocalDate startDate, LocalDate endDate, Integer pageNumber) {
		Pageable pageable = PageRequest.of(pageNumber - 1, 8, Sort.Direction.ASC, "date");
		Page<Attendance> attendancePage = attendanceRepository.findByDateBetweenAllEmpAttendance(startDate, endDate,
				pageable);
		Page<AttendanceDTO> result = attendancePage.map(attendance -> new AttendanceDTO(attendance));
