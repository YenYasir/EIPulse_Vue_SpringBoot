   ...
   <div class="app-container">
      <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
         ...
         <el-form-item label="任務組名" prop="jobGroup">
            <el-select v-model="queryParams.jobGroup" placeholder="請選擇任務組名" clearable style="width: 200px">
               <el-option
         ...
         <el-form-item label="任務狀態" prop="status">
            <el-select v-model="queryParams.status" placeholder="請選擇任務狀態" clearable style="width: 200px">
               <el-option
      ...
      <el-dialog :title="title" v-model="open" width="820px" append-to-body>
         <el-form ref="jobRef" :model="form" :rules="rules" label-width="120px">
            <el-row>
                     ...
                     <template #label>
                        <span>
                           ...
                           <el-tooltip placement="top">
                              <template #content>
                                 <div>
                                    Bean調用示例：eipulseTask.ryParams('ry')
                                    <br />Class類調用示例：com.ruoyi.quartz.task.EipulseTask.ryParams('ry')
                                    <br />參數說明：支持字串，布林類型，長整數，浮點數，整數
                                 </div>
                              </template>
               ...
               <el-col :span="12">
                  <el-form-item label="是否並" prop="concurrent">
     ...
     <el-dialog title="Cron表達式生成器" v-model="openCron" append-to-body destroy-on-close>
       <crontab ref="crontabRef" @hide="openCron=false" @fill="crontabFill" :expression="expression"></crontab>
     </el-dialog>

      <!-- 任務日誌詳細 -->
      <el-dialog title="任務詳細" v-model="openView" width="700px" append-to-body>
...
const data = reactive({
  ...
  rules: {
    jobName: [{ required: true, message: "任務名稱不能為空", trigger: "blur" }],
    invokeTarget: [{ required: true, message: "調用目標字串不能為空", trigger: "blur" }],
    cronExpression: [{ required: true, message: "cron執行表達式不能為空", trigger: "change" }]
  }
});
...
function handleStatusChange(row) {
  ...
  proxy.$modal.confirm('確認要"' + text + '""' + row.jobName + '"任務嗎?').then(function () {
...
function handleRun(row) {
  proxy.$modal.confirm('確認要立即執行一次"' + row.jobName + '"任務嗎?').then(function () {
    return runJob(row.jobId, row.jobGroup);
  }).then(() => {
    proxy.$modal.msgSuccess("執行成功");})
  .catch(() => {});
}
/** 任務詳細資訊 */
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否確認刪除定時任務編號為"' + jobIds + '"的數據項?').then(function () {
