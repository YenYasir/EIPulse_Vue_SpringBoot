<template>
  <div class="card text-center div1">
    ...
    <div class="card-body">
      <form @submit.prevent="productsSave">
        <div v-for="(n, index) in formCount" :key="n">
          <span>No.{{ n }}</span>
          <product-input
            @product-update="(product) => updateProduct(product, index)"
          ></product-input>
        </div>
        <div class="row mb-4">
          <button @click.prevent="addFrom" class="btn btn-outline-primary">
            +
          </button>
        </div>
    ...
    <div class="card-footer text-body-secondary">©2023 EIPulse</div>
  </div>
</template>
    ...
    async productsSave() {
      try {
        const response = await axios.post(
          "http://localhost:8090/eipulse/products",
          this.products
        );
        await Swal.fire({
          title: "產品新增成功",
