...
export default {
  ...
  methods: {
    ...
    loadProducts() {
      axios.get(`${URL}products/${this.currentPage}`).then(res => {
    ...
    async changeStatus(productType){
      ...
      try {
        if(result.isConfirmed){
          const res = await  axios.put(`${URL}product/status`,{
            id:productType.id
          })
          this.loadProducts();
          Swal.fire({
    ...
    async editSubmit() {
      try {
        const response = await axios.put(`${URL}product`, this.editProduct)
        Swal.fire({
          title: '修改成功',
          timer: 1000,
          timerProgressBar: true,
...
<template>
  <div class="card text-center  rounded border-0 shadow-sm">
