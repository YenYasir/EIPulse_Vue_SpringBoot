  ...
  methods: {
    async handleSubmit() {
      try {
        ...
        this.$emit('login-success', this.empId)
      } catch (e) {
        await Swal.fire({
          title: '登入失敗',
          text: '請確認工號與密碼',
    ...
    toggleForgetPassword() {
      this.showIsForget = !this.showIsForget;
    },
    async sendEmail() {
...
<template>
  <div class="card text-center div1" v-if="!showIsForget " @forget='toggleForgetPassword'>
  ...
  <div class="card text-center div1" v-else>
    <div class="card-header">
      忘記密碼
    </div>
    <div class="card-body">
